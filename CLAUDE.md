...
# then auto-tests the best-by-val EMA ckpt on the held-out 10% — logging BOTH BP truth
# sources to SwanLab (test/* per-beat-on-true-wave, test/cuff_* CSV cuff) + test_metrics.json.
...
# Standalone eval of a finetuned checkpoint on the CalFree held-out 10% test split
# (finetune now also auto-tests at train end; this re-runs it on demand). infer.sh
# loads finetune.yaml so the split matches finetune's fixed-seed 8:1:1. Args: <checkpoint> [--nproc N] [extra args].
...
- **`finetune.yaml`** (inherits `gpu.yaml`) — domain adaptation on CalFree. Spells out its own `training` block (tune finetune hyperparams independently). **`data.finetune: true`** splits the CalFree `test_npy` 8:1:1 (per-segment, fixed seed) into train/val/test. After training, **`run_test_after_train: true`** auto-tests the best-by-val EMA model on the held-out 10% — logging BOTH clinical truth sources separately (`test/*` per-beat-on-true-wave, `test/cuff_*` CSV cuff) + `test_metrics.json`; `infer.sh` re-runs the same eval standalone. Weights init from a pretrained checkpoint via `--init-ckpt` (→ `training.init_from_ckpt`); optimizer/epoch/step reset.
...
- **`run_test_after_train`** (gpu.yaml **false**, finetune.yaml **true**): after training finishes (normal end or early stop — NOT a `max_steps` interrupt), `Trainer.run_test()` evaluates the **best-by-val EMA** model on the `test` split (DDP-sharded, shared `_distributed_eval`), logs to SwanLab + writes `output/<ts>/test_metrics.json` inside `train()` before `close()` (same SwanLab run); `test_max_segments > 0` caps it. **`run_test()` forces both clinical truth sources**: it builds the test set with `eval_true_source: both`, so PRED per-beat is scored against the true-wave per-beat (`test/*`) AND the CSV cuff label (`test/cuff_*`), logged separately (falls back to waveform-only if the CSV is absent — `_distributed_eval` gathers the cuff per segment only when present). Pretraining leaves it off (CalFree untouched); finetune runs it on the held-out 10%. `bash infer.sh <ckpt>` re-runs the same eval standalone.
- **Finetune flow (`data.finetune: true`)** repurposes the CalFree `test_npy`: a fixed-seed **per-segment** 8:1:1 split (`finetune_val_fraction`/`finetune_test_fraction`, default 0.1/0.1) makes train/val/**test** all read CalFree, non-overlapping. `split_mode` is ignored (no subject split). The pretrained model is loaded via **`--init-ckpt`** → `training.init_from_ckpt` (`Trainer._maybe_init_from_ckpt`): it copies `model` (+ `model_ema`) weights only, then trains fresh (optimizer/epoch/step reset). Skipped when resuming an interrupted run. ⚠️ The split is per-segment, so the same subject can appear in both finetune-train and the held-out test (auto-tested at train end, also re-runnable via `infer.sh`) → that test number is optimistic, not subject-disjoint.
