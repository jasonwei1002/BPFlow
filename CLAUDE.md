...
# --tag <suffix> appends _<suffix> to every pinned run/dir/SwanLab-group name (both
# run_pulsedb.sh and run_uci.sh) — use it to keep an ablation run's dirs distinct.
...
bash run_pulsedb.sh --tag abljoint_late --pt "model.stream_fusion=late"  # --tag suffixes all dir/run/group names (ablation id)

# Pull SwanLab metrics for the latest run (project "bpflow"; skill: swanlab-skill)
swanlab api run list badwoman/bpflow --page_size 10        # page_size in {10,12,15,20,24,27,50,100}; newest first, copy its run id
...
- **`pulsedb.yaml`** (inherits `base.yaml`) — real **pretraining** (epochs 1000 as a ceiling; early-stop + plateau decay end runs sooner; global batch 1024, lr 3e-4, SwanLab cloud). Uses `split_mode: segment` (random per-segment train/val). Also `val_eval_fraction: 0.1` (validate on a representative stride-sampled 10% of val) and **`run_test_after_train: false`** — pretraining never touches CalFree (reserved for finetune).
...
- **`batch_size` is the GLOBAL batch** (summed across all GPUs). The trainer auto-divides by `num_gpus` to get the per-GPU loader batch (e.g. `1024 / 8 = 128`); a non-divisible value is floored to `global // world_size` with a rank-0 warning. `val_batch_size` is global the same way (`-1` = reuse the train global batch). So the global batch is **fixed regardless of GPU count**, and `lr` (sqrt-scaled for the global batch in `pulsedb.yaml`) no longer tracks GPU count. Set it in config as the TOTAL batch, not per-GPU — changing `--nproc` keeps the same global batch / lr.
