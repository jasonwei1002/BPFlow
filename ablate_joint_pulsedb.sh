...
#
# Runs the flagship ECG+PPG->ABP SPECIALIST on PulseDB through the full
...
# (PulseDB / specialist / ecg_ppg2abp). Only model.stream_fusion is flipped.
#
# Baseline (joint) to compare against, if not already run:
#   bash run_pulsedb.sh --tag base --pt "data.tasks=[ecg_ppg2abp] data.task_probs=[1.0]" --ft "data.tasks=[ecg_ppg2abp] data.task_probs=[1.0]"
#
# Usage: bash ablate_joint_pulsedb.sh [--nproc N] [extra run_pulsedb.sh args]
set -euo pipefail
cd "$(dirname "$0")"
bash run_pulsedb.sh --tag abljoint_late \
