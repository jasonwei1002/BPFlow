...
def sample_abp(
    ...
    *,
    ...
    eval/param state. The null condition is only built when CFG is active
    (``cfg_strength > 1``); otherwise ``ode_wrapper`` ignores it.
    """
    cond_patches = cond_patches.to(device)
    bs = cond_patches.shape[0]
    conditions = model.preprocess_conditions(cond_patches)
    empty = model.get_empty_conditions(bs) if cfg_strength > 1.0 else conditions
    x0 = (
        torch.randn(
...
def flow_matching_loss(
    ...
    *,
    ...
    training under DDP (gradient sync) and the raw module otherwise.
    """
    ...
    pred = model(xt, cond_patches, t_sh)
